...
Describe "Get-PowerPlatformCheckerAppModelDriven" {
    BeforeAll {
    ...
    It "returns app metadata from appmodule and sitemap" {
        InModuleScope PowerPlatformChecker {
            param($SolutionPath)
            $apps = Get-PowerPlatformCheckerAppModelDriven -SolutionPath $SolutionPath
    ...
    It "handles invalid model-driven metadata fixtures gracefully" {
        InModuleScope PowerPlatformChecker {
            param($SolutionPath)
            $warnings = @()
            $apps = Get-PowerPlatformCheckerAppModelDriven -SolutionPath $SolutionPath -Name "invalid-model-app" -WarningVariable warnings -WarningAction SilentlyContinue
    ...
    It "keeps app output when sitemap metadata is invalid" {
        InModuleScope PowerPlatformChecker {
            param($SolutionPath)
            $warnings = @()
            $apps = Get-PowerPlatformCheckerAppModelDriven -SolutionPath $SolutionPath -Name "invalid-sitemap-app" -WarningVariable warnings -WarningAction SilentlyContinue
    ...
    It "discovers fixture web resources from entity form xml without appmodule js components" {
        $app = & (Get-Module PowerPlatformChecker) { param($path) Get-PowerPlatformCheckerAppModelDriven -SolutionPath $path -Name "ppc_ModelApp" | Select-Object -First 1 } $script:solutionPath
...
"@ | Set-Content -Path (Join-Path $webResourcePath "Form.js.data.xml") -Encoding utf8BOM
        ...
        $webResources = & (Get-Module PowerPlatformChecker) { param($path) Get-PowerPlatformCheckerAppModelDriven -SolutionPath $path | Select-Object -First 1 | Select-Object -ExpandProperty WebResources } $testRoot
...
"@ | Set-Content -Path (Join-Path $webResourcePath "Form.js.data.xml") -Encoding utf8BOM
        ...
        $app = & (Get-Module PowerPlatformChecker) { param($path) Get-PowerPlatformCheckerAppModelDriven -SolutionPath $path | Select-Object -First 1 } $testRoot
